...
if [[ ${FILENAME} =~ \.gz$ ]];then
  ...
  zipped=1
  gzip -dk --force ${FILENAME}
  FILENAME=${FILENAME%.mps.gz}.mps
fi

echo ">>> Executing: ${EXECNAME} -f ${FILENAME} -p ${SETFILENAME} --tlim ${TIMELIMIT} --presolve.randomseed=${SEED}"
eval "${EXECNAME} -p ${SETFILENAME} -f ${FILENAME} --tlim ${TIMELIMIT} --presolve.randomseed=${SEED}" 2>> "${ERRFILE}" | tee -a "${OUTFILE}"

CERTIFICATE=${FILENAME%.mps}.vipr
TMP_CERTIFICATE=${FILENAME%.mps}-temp.vipr

echo ">>> Executing: ${VIPR} ${CERTIFICATE}"
eval "${VIPR} ${CERTIFICATE}" 2>> "${ERRFILE}" | tee -a "${OUTFILE}"

rm -r ${TMP_CERTIFICATE}

if [[ "$zipped" -eq 1 ]]; then
  echo ">>> Deleting extracted: ${FILENAME}"
  rm -r ${FILENAME}
fi
